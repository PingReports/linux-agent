...
#
# SECURITY: agent.sh sources this file (`. "$AGENT_CONF"`), so any value
# we interpolate must be safe inside double quotes. PR_AGENT_NAME defaults
# to $(hostname); cloud-init / DHCP option 12 / VM templates can put
# arbitrary characters into the hostname including `"; rm -rf /; "`.
# Without escaping, a malicious hostname becomes executable shell on the
# next push. PR_AGENT_TAGS is operator-supplied and follows the same
# rule. We use `printf %q` (bash) when available and a portable fallback
# otherwise.
sh_quote() {
  # POSIX-safe shell-quote: wrap in single quotes, escape embedded
  # single quotes as '"'"'. Output is always safe inside both double-
  # quoted and unquoted contexts in a sourced config file.
  printf "'%s'" "$(printf '%s' "$1" | sed "s/'/'\\\\''/g")"
}
umask 077
{
  printf '# PingReports linux agent — generated by install.sh, ok to edit.\n'
  printf 'PR_AGENT_ID=%s\n'      "$(sh_quote "$PR_AGENT_ID")"
  printf 'PR_AGENT_TOKEN=%s\n'   "$(sh_quote "$PR_AGENT_TOKEN")"
  printf 'PR_AGENT_NAME=%s\n'    "$(sh_quote "$PR_AGENT_NAME")"
  printf 'PR_AGENT_VERSION=%s\n' "$(sh_quote "$PR_AGENT_VERSION")"
  printf 'PR_AGENT_TAGS=%s\n'    "$(sh_quote "$PR_AGENT_TAGS")"
  printf 'PR_INGEST_URL=%s\n'    "$(sh_quote "$PR_INGEST_URL")"
  printf 'PR_DISK_PATHS=%s\n'    "$(sh_quote "${PR_DISK_PATHS:-/}")"
  printf 'PR_NET_IFACES=%s\n'    "$(sh_quote "${PR_NET_IFACES:-}")"
  printf 'PR_HTTP_TIMEOUT=%s\n'  "$(sh_quote "${PR_HTTP_TIMEOUT:-30}")"
  printf 'PR_QUEUE_MAX=%s\n'     "$(sh_quote "${PR_QUEUE_MAX:-50}")"
} > "$AGENT_CONF.new"
